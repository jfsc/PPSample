...
public class PetersonTree{
	...
	public void PTlock(){
		...
		while (start>=0){
			...
			System.out.println(Thread.currentThread().getId()+": DISPUTANDO RC NO NÓ: "+this.lockLevel[start][i].hashCode());
			this.lockLevel[start][i].lock(me,this.lockLevel[start][i].hashCode());
			start--;
			System.out.println(Thread.currentThread().getId()+": CONSEGUI DESCER 1 NÓ ATÉ A RAIZ");
		}
	...
	public void PTunlock(){
		...
		for(int j=(path.size() - 1); j >= 1; j--){
			...
			System.out.println(Thread.currentThread().getId()+": ESTOU SAINDO DA RC NO NÓ "+this.lockLevel[start][path.get(j)].hashCode());
			start++;
